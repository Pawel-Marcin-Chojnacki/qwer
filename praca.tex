...
\begin{figure}[ht]
 	\centering
 			\includegraphics[resolution=100, scale=0.5]{WRN.png}
 		\caption{Wizualizacja architektury szerokiej sieci szczątkowej.}
 \end{figure}

Architektura szerokiej sieci szczątkowej (ang. Wide Residual Network) jest rozszerzeniem pomysłu ResNet oraz ResNeXt. Sieci oparte o architekturę szczątkową są zdolne do skalowania się do tysięcy warstw ukrytych bez zjawiska znikającego gradientu. Autorzy architektury osiągnęli wzrost jakości sieci dokładając tysiące warstw. %Jednakże po przekroczeniu progu kilkuset warstw, dokładność sieci zwiększa się o coraz mniejsze ułamki procenta. 
...
%\subsection{Nieprawidłowe klasyfikacje}
...
\subsection{DenseNet}
Najpopularniejsza architektura dla uczenia zdjęć. Jest bardzo szybka w trenowaniu nowych modeli i posiada implementację w każdej bibliotece głębokiego uczenia. Sieć poprawnie rozdzieliła przestrzeń klas dla zbioru obrazów. Uzyskała wynik na poziomie błędu 3\%, jest to światowej klasy model uzyskany już po kilku próbach dostrojenia parametrów.
...
\begin{center}
    ...
    Czas & 10,2s & 12,4s & 2612s & 457s \\ \hline
    Dokładność & 100\% & 93,7\% & 100\% & 93,7\% \\ \hline
    Błąd treningowy & 8,5\% & 8\% & 7,5\% & 8\% \\ \hline
    Błąd walidacyjny & 10,2\% & 10\% & 3,7\% & 10\% \\ \hline
    \end{tabular}
\end{center}

\section{Podsumowanie}
Pierwszym wnioskiem, jaki nasuwa się po przeprowadzeniu testów wydajności, jest: nie zaczynać przygody z głębokimi sieciami neuronowymi bez dostępu do procesora graficznego o dużej mocy obliczeniowej. Oczekiwanie na wyniki używając jedynie jednostki wyposażonej w CPU zniechęci każdego do eksperymentów, a to eksperymenty są podstawą doboru świetnych modeli. Wybierając kartę graficzną należy się kierować odpowiednio dużą pamięcią. Spora część zbioru danych uczących jest ładowana do pamięci GPU.

Wpływ wybranej architektury na dokładność sieci ma malejące znaczenie z każdą nową siecią. Dzieje się tak, ponieważ blisko stuprocentowa skuteczność nie daje dużego pola manewru. Zbliżając się do blisko stuprocentowej skuteczności rozpoznawania obrazów trudno jest stwierdzić, że istnieją wyzwania w tej dziedzinie. Należałoby się raczej skupić na bezpieczeństwie sieci i zabezpieczeniami przed próbami oszukania. Ataki stają się popularne, kiedy agenci automatyczni zostają wdrożeni na szeroką skalę.
...
Architektury sieci neuronowych przeszły wielką ewolucję w ostatniej dekadzie po latach powolnego rozwoju. Wymyślność kolejnych zwycięzców zawodów pozwoliła na korzystanie z wyszukanych modeli. Cieszy również fakt obecnej popularności tematyki uczenia maszynowego. Mnogość aktywnie utrzymywanych bibliotek daje wielkie nadzieje na dalszy dynamiczny rozwój dziedziny.

Wyniki testowanych w niniejszej pracy architektur, pozwalają wyciągnąć wnioski dotyczące przyszłości. 
Wydajność w klasyfikacji obrazów dla dużych zbiorów danych medycznych przewyższa dokładność pojedynczego radiologa. Wykorzystanie systemów uczących na szeroką skalę, przykładowo przy wsparciu pracy radiologa, jest już stosowane w chińskich placówkach medycznych w Pekinie. 
Niestety do wykorzystania systemu jako samodzielnego agenta decyzyjnego, należy rozwiązać problem uzasadnienia decyzji. W prawodastwie rzymskim (w tym również polskim) prócz samego czynu równie ważne okazują się motywacje. Tak długo jak system nie będzie w stanie argumentować swoich decyzji, nawet model o stuprocentowej skuteczności nie będzie mógł podejmować sprawczych decyzji.

%Firmy produkujące biblioteki głębokiego uczenia maszynowego, publikują od czasu do czasu notatki prasowe, w których oznajmiają szumnie informacje o nadchodzącej rewolucji. Głównym motywem jest sprzedaż usług Sztucznej Inteligencji jako usługi na żądanie w chmurze obliczniowej. 
